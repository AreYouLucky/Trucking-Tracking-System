  ...
  <v-data-table :headers="headers" :items="deliveries" :search="search" :loading="loading" v-model:sort-by="sortBy">
    <template v-slot:top>
      <v-toolbar color="blue">
        <v-spacer></v-spacer>
        <v-text-field v-model="search" clearable rounded="xl" theme="light" density="comfortable" hide-details
          placeholder="Search" prepend-inner-icon="mdi-magnify" style="max-width: 400px;" variant="solo"
          class="mr-2"></v-text-field>
      </v-toolbar>
    </template>
    <template v-slot:item.lname="{ item }">
      <v-btn variant="tonal" color="warning" prepend-icon="mdi-check-circle"
        v-if="!item.driver_id">to be assigned</v-btn>
      <span v-else>{{ item.lname }}, {{item.fname  }} ({{ item.contact_no }})</span>
    </template>
    <template v-slot:item.actions="{ item }">
      <v-btn icon="mdi-car-connected" variant="text" color="primary"
        :href="`/driver-delivery-location/${item.driver_id}`" target="_blank" v-if="item.is_delivered < 3"></v-btn>
    ...
    <template v-slot:item.status="{ item }">
      ...
      <v-btn variant="tonal" color="orange" prepend-icon="mdi-ticket-confirmation" v-if="item.is_delivered === 3"
        @click="popConfirm(item.delivery_id)">Waiting Confirmation</v-btn>
